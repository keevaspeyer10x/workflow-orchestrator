...
1. **Create a new branch** for this work:
   ```bash
   git checkout -b claude/t1-manual-1
   ```
...
5. **Report completion** by providing:
   - Branch name: `claude/t1-manual-1`
...
```bash
orchestrator prd report-complete manual-1b984bac --branch <branch-name> --commit <commit-sha>
```

Or if the task failed:

```bash
orchestrator prd report-failed manual-1b984bac --error "Description of what went wrong"
```
