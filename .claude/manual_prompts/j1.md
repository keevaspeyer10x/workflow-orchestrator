...
1. **Create a new branch** for this work:
   ```bash
   git checkout -b claude/t1-manual-4
   ```
...
5. **Report completion** by providing:
   - Branch name: `claude/t1-manual-4`
...
```bash
orchestrator prd report-complete manual-4936c891 --branch <branch-name> --commit <commit-sha>
```

Or if the task failed:

```bash
orchestrator prd report-failed manual-4936c891 --error "Description of what went wrong"
```
