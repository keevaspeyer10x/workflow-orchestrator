...
1. **Create a new branch** for this work:
   ```bash
   git checkout -b claude/t1-manual-7
   ```
...
5. **Report completion** by providing:
   - Branch name: `claude/t1-manual-7`
...
```bash
orchestrator prd report-complete manual-7d5ad766 --branch <branch-name> --commit <commit-sha>
```

Or if the task failed:

```bash
orchestrator prd report-failed manual-7d5ad766 --error "Description of what went wrong"
```
