...
1. **Create a new branch** for this work:
   ```bash
   git checkout -b claude/t1-manual-9
   ```
...
5. **Report completion** by providing:
   - Branch name: `claude/t1-manual-9`
...
```bash
orchestrator prd report-complete manual-9204c197 --branch <branch-name> --commit <commit-sha>
```

Or if the task failed:

```bash
orchestrator prd report-failed manual-9204c197 --error "Description of what went wrong"
```
