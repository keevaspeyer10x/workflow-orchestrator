...
1. **Create a new branch** for this work:
   ```bash
   git checkout -b claude/t1-manual-e
   ```
...
5. **Report completion** by providing:
   - Branch name: `claude/t1-manual-e`
...
```bash
orchestrator prd report-complete manual-e581478a --branch <branch-name> --commit <commit-sha>
```

Or if the task failed:

```bash
orchestrator prd report-failed manual-e581478a --error "Description of what went wrong"
```
