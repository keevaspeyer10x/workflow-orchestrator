...
1. **Create a new branch** for this work:
   ```bash
   git checkout -b claude/t1-manual-c
   ```
...
5. **Report completion** by providing:
   - Branch name: `claude/t1-manual-c`
...
```bash
orchestrator prd report-complete manual-c460fa20 --branch <branch-name> --commit <commit-sha>
```

Or if the task failed:

```bash
orchestrator prd report-failed manual-c460fa20 --error "Description of what went wrong"
```
