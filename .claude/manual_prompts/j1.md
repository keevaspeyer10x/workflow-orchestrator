...
1. **Create a new branch** for this work:
   ```bash
   git checkout -b claude/t1-manual-b
   ```
...
5. **Report completion** by providing:
   - Branch name: `claude/t1-manual-b`
...
```bash
orchestrator prd report-complete manual-b1bd8c1b --branch <branch-name> --commit <commit-sha>
```

Or if the task failed:

```bash
orchestrator prd report-failed manual-b1bd8c1b --error "Description of what went wrong"
```
