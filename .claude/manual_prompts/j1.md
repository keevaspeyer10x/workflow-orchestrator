...
1. **Create a new branch** for this work:
   ```bash
   git checkout -b claude/t1-manual-a
   ```
...
5. **Report completion** by providing:
   - Branch name: `claude/t1-manual-a`
...
```bash
orchestrator prd report-complete manual-afc94a1f --branch <branch-name> --commit <commit-sha>
```

Or if the task failed:

```bash
orchestrator prd report-failed manual-afc94a1f --error "Description of what went wrong"
```
