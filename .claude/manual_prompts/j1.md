...
1. **Create a new branch** for this work:
   ```bash
   git checkout -b claude/t1-manual-0
   ```
...
5. **Report completion** by providing:
   - Branch name: `claude/t1-manual-0`
...
```bash
orchestrator prd report-complete manual-06f92233 --branch <branch-name> --commit <commit-sha>
```

Or if the task failed:

```bash
orchestrator prd report-failed manual-06f92233 --error "Description of what went wrong"
```
